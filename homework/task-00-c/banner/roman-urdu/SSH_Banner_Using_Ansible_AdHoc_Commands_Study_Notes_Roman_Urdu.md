...
9. [Step 5 — `lineinfile` se `sshd_config` configure karein](#9-step-5--lineinfile-se-sshd_config-configure-karein)
   - [SSH Banner regex ki tafseel](#91-ssh-banner-regex-ki-tafseel)
...
### 9.1 SSH Banner regex ki tafseel

Command mein yeh regular expression use ho rahi hai:

```regex
^\s*#?\s*Banner\s+.*$
```

Yeh active aur commented dono tarah ki aam `Banner` directives ko match karti hai.
...
Anchors `^` aur `$` pattern ko unrelated text mein maujood sirf `Banner` lafz match karne se rokte hain.

#### Match milne par `lineinfile` kya karega?

- Ek line match ho to Ansible usay `Banner /etc/ssh/banner.txt` se replace karega.
- Koi line match na ho to Ansible required line ko aam tor par file ke end mein add karega.
- Kai lines match hon to `lineinfile` aam tor par aakhri matching line change karta hai. Duplicate directives inspect karein aur har matching line change karni ho to `replace` module use karein.
- Second run par desired line pehle se correct ho to Ansible ko `changed=false` report karna chahiye.

Change se pehle matching lines dikhane ke liye:
...
`grep` example POSIX character classes use karta hai, jab ke Ansible ka `regexp` Python-style `\s` use karta hai. Dono apne tools mein whitespace ki same requirement ko express karte hain.
...
11. Banner regex mein `#?` ka kya matlab hai?
12. Is pattern mein `^` aur `$` kyun useful hain?
13. Agar kai lines `lineinfile` regex ko match karein to kya hota hai?
