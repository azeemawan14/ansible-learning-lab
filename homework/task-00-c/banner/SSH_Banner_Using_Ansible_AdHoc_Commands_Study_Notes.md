...
9. [Step 5 — Configure `sshd_config` with `lineinfile`](#9-step-5--configure-sshd_config-with-lineinfile)
   - [Regex deep dive for the SSH Banner](#91-regex-deep-dive-for-the-ssh-banner)
...
### 9.1 Regex deep dive for the SSH Banner

The command uses this regular expression:

```regex
^\s*#?\s*Banner\s+.*$
```

It matches a typical `Banner` directive whether the line is active or commented out.
...
The anchors `^` and `$` prevent the pattern from matching unrelated text that merely contains the word `Banner`.

#### What `lineinfile` does with the match

- If one line matches, Ansible replaces it with `Banner /etc/ssh/banner.txt`.
- If no line matches, Ansible adds the required line, normally at the end of the file.
- If several lines match, `lineinfile` normally changes the last matching line. Inspect duplicate directives and use the `replace` module when every matching line must change.
- On a second run, the desired line already matches and Ansible should report `changed=false`.

Show the matching lines before making a change:
...
The `grep` example uses POSIX character classes, while the Ansible `regexp` uses Python-style `\s`. Both patterns express the same whitespace requirement in their respective tools.
...
11. What does `#?` mean in the Banner regular expression?
12. Why are `^` and `$` useful in this pattern?
13. What happens when several lines match a `lineinfile` regular expression?
